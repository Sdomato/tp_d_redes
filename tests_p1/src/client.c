...
int do_wrq(int sockfd,
           ...
           const char *remote_filename)
{
    // ---------------- Validación del nombre remoto ----------------
    if (!remote_filename) {
        fprintf(stderr, "WRQ: Filename nulo.\n");
        return -1;
    }

    size_t name_len = strlen(remote_filename);

    // Tamaño máximo permitido por buffer
    if (name_len == 0 || name_len >= MAX_FILENAME) {
        fprintf(stderr, "WRQ: Filename demasiado largo para el buffer.\n");
        return -1;
    }

    // Demasiado corto (<4) siempre inválido (T15)
    if (name_len < 4) {
        fprintf(stderr,
                "WRQ: Filename debe tener al menos 4 caracteres.\n");
        return -1;
    }

    // Contar si tiene extensión (punto) y validar ASCII
    int has_dot = 0;
    for (size_t i = 0; i < name_len; ++i) {
        unsigned char c = (unsigned char)remote_filename[i];

        // No aceptamos caracteres no ASCII (>=128)
        if (c > 127) {
            fprintf(stderr,
    ...
    // Si no tiene punto (sin extensión) y es >10, se rechaza (T16)
    if (name_len > 10 && !has_dot) {
        fprintf(stderr,
                "WRQ: Filename sin extensión no puede superar 10 caracteres.\n");
        return -1;
    }
